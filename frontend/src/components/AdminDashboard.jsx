...
function AdminDashboard() {
    ...
    useEffect(() => {
        const fetchComplaints = async () => {
            try {
                // Make the GET request to your backend's API endpoint
                const response = await axios.get('http://localhost:5000/api/complaints');
                // Store the fetched data in the state
                setComplaints(response.data);
            } catch (err) {
                // If an error occurs, store the error message
                setError('Failed to fetch complaints. Make sure the backend server is running.');
                console.error(err);
            } finally {
    ...
    const handleStatusChange = async (id, newStatus) => {
        try {
            // Make a PUT request to update the status for a specific complaint ID
